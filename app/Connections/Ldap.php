...
use Adldap\Connections\LdapFunctionSupportTrait;
...
class Ldap implements ConnectionInterface
{
    ...
    {ldap_add($this->getConnection(), "cn=Test Group,dc=silent,dc=com", ['cn' => 'Test Group', 'objectClass' => ['groupofuniquenames','top'], 'uniquemember' => 'uid=admin,cn=AdminUsers,dc=silent,dc=com']);

        dd('herooe');
        return ldap_add($this->getConnection(), $dn, $entry);
    }

    public function ldap_createGroup($object_name, $dn, $members, $ldap_conn)
    {
        $addgroup_ad['cn']=$object_name;
        $addgroup_ad['sn']=$object_name;
        $addgroup_ad['objectClass'][0] = "top";
        $addgroup_ad['objectClass'][1] ="person";
        // $addgroup_ad['groupType']="1";
